...
function renderControls() {
  ...
  const canTriggerCurrentAi = currentType === "minimax" ? true : canUseApiAi();
...
async function runAiTurn(options = {}) {
  ...
  if (!strategy) return;
  if (strategy === "api" && !canUseApiAi()) {
    pushLog("error", "API AI 配置不完整，请先填写 Endpoint 和 Model。");
    return;
  }

  state.ui.pending = true;
  render();
  pushLog("info", `${currentPlayer.label} 开始${strategy === "minimax" ? "进行本地搜索" : "请求 API AI"}...`);

  try {
    const decision = strategy === "minimax"
      ? await requestMinimaxDecision(cloneGame(state.game), currentPlayer.id)
      : await requestAiActionDecision(cloneGame(state.game), currentPlayer.id);
...
function resolveTurnAiStrategy(currentType, force) {
  if (currentType === "minimax" || currentType === "api") return currentType;
